...
ip=`cat /etc/cron.daily/radio-update.sh | grep ip= | cut -d '"' -f2`

hora=`cat /etc/cron.daily/radio-update.sh | grep hora= | cut -d '"' -f2`
...
data_sql=`stat /var/www/html/radio.sql | grep Modify | tail -n1 | cut -d " " -f2`

data_zip=`stat /var/www/html/radio.zip | grep Modify | tail -n1 | cut -d " " -f2`
...
verifica=`cat /etc/cron.daily/radio-update.sh | grep ip= | cut -d '"' -f2 | cut -d "." -f4`
if [ "$verifica" -eq 226 ]
then
	...
	/etc/cron.daily/transfere.exp $ip
else
	# Envia o arquivo recem criado para o server
	rsync -aq --timeout=180 --password-file=/etc/radio.update ${locate}/*.zip rsync://${rede}@${ip}:873:/${rede}_logs
fi
