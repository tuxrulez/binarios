...
> ${path_base}/completo.txt
> ${path_base}/verificar_conteudo.txt
...
echo "select t1.loja from lojas as t1, redes as t2, conteudos as t3 where t3.data='${data}' and t3.lojas_idloja=t1.idloja and t1.redes_idrede=t2.idrede and t3.statuslog_idstatus=3 and t1.redes_idrede=${id_rede} order by t1.loja;" | mysql -h${ip} -u${user} ${db} -p${pw} | grep -v 'loja' >> ${path_base}/completo.txt
...
# Rotina que encontra as maquinas com sinal de satelite mas nao estap com conteudo completo
cat ${path_base}/satelite_on.txt | cut -d ":" -f1 | while read lojas
do
	verifica=`cat ${path_base}/completo.txt | grep ${lojas}`
	if [ "${verifica}" == "" ]
	then
		echo "${lojas}" >> ${path_base}/verificar_conteudo.txt
	fi
done
