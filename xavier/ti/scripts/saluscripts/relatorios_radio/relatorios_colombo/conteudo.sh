...
path_relatorio="/ti/scripts/saluscripts/relatorios_radio/relatorios_colombo"
...
ls ${path_sistema}/${rede}/sql/ | grep -v '^000' | grep -v 'AProgramacao' | while read loja
do
...
total=`ls ${path_sistema}/${rede}/sql/ | grep -v '^000' | grep -v 'AProgramacao' | wc -l`
