...
arquivo="Lojas_Colombo - Ofertas Ate 310310-13.mp3"
...
ls /home/radio/${rede}/sql/ | grep 'PR_0159' | while read loja
do
	log=`ls /logs_novo/radio/${rede}/*status* | grep $loja`	
	if [ "$log" != "" ]
	then
        	arqu=`zcat $log | grep 'Lojas_Colombo - Ofertas Ate 310310-13.mp3'`
		if [ "$arqu" != "" ]
		then
...
echo " "                                                                        >> ${rede}_estatisticas
echo " Total de Lojas       = "`ls /home/radio/${rede}/sql/ | grep 'PR_0159' | wc -l`  >> ${rede}_estatisticas
echo " Lojas Atualizadas    = "`cat ${rede}_lojas_com_conteudo | wc -l`         >> ${rede}_estatisticas

total=`ls /home/radio/${rede}/sql/ | grep 'PR_0159' | wc -l`
