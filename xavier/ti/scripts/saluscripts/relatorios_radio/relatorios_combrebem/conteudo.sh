...
path_relatorio="/ti/scripts/saluscripts/relatorios_radio/relatorios_comprebem"
...
ls ${path_sistema}/${rede}/sql/ | grep -v '^000' | grep -v 'AAAteste' | grep -v 'AProgramacao' | grep -v 'Eventos' | while read loja
do
...
total=`ls ${path_sistema}/${rede}/sql/ | grep -v '^000' | grep -v 'AAAteste' | grep -v 'AProgramacao' | grep -v 'Eventos' | wc -l`
