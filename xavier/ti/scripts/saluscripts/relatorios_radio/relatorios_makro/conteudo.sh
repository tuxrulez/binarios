...
path_relatorio="/ti/scripts/saluscripts/relatorios_radio/relatorios_makro"
...
ls ${path_sistema}/${rede}/sql/ | grep -v '^000' | grep -v 'AProgramacao' | grep -v 'Adega' | while read loja
do
...
total=`ls ${path_sistema}/${rede}/sql/ | grep -v '^000' | grep -v 'AProgramacao' | grep -v 'Adega' | wc -l`
